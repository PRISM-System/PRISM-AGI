...
class ToolRegister {
    ...
    async handleSubmit() {
        try {
            ...
            // API 요청
            const response = await fetch('/api/clients/user/tools/', {
                method: 'POST',
                headers: {
    ...
    collectFormData() {
        ...
        // JSON 필드 파싱
        try {
            data.input_schema = JSON.parse(formData.get('input_schema') || '{}');
        } catch (error) {
        ...
        // 엔드포인트 객체 구성
        data.endpoint = {
            url: formData.get('endpoint_url').trim(),
            method: formData.get('endpoint_method'),
            timeout_s: parseInt(formData.get('endpoint_timeout')) || 10
        };

        // 헤더 파싱 (선택사항)
        const headersValue = formData.get('endpoint_headers').trim();
        if (headersValue) {
            try {
                data.endpoint.headers = JSON.parse(headersValue);
            } catch (error) {
    ...
    validateFormData(data) {
        ...
        if (!data.input_schema) errors.push('올바른 JSON 스키마를 입력해주세요.');
        if (!data.endpoint.url) errors.push('API URL을 입력해주세요.');
        if (!data.endpoint.method) errors.push('HTTP 메서드를 선택해주세요.');
